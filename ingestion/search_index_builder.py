...
class SearchIndexBuilder:
    ...
    def load(self, loader, _bundle=None):
        ...
        # 9. PII attributes
        for r in q("""SELECT pii_attribute, pii_components, pii_sensitivity_category
                      FROM pii_classifications"""):
            rows.append({
                "artifact_key": f"pii:{r['pii_attribute']}".lower()[:400],
                "module": "pii", "kind": "pii", "name": r["pii_attribute"],
                "body_text": _body(r["pii_attribute"], r["pii_components"], r["pii_sensitivity_category"]),
                "subtitle": f"{r['pii_sensitivity_category'] or 'PII'} · {r['pii_components'] or ''}"[:1000],
