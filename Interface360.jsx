...
export default function Interface360({ t, selection }) {
  ...
  useEffect(() => {
    // Forward every active filter to the API. Multi-selects go as arrays
    // (api.js joins them to CSV; the route splits into exact-match IN(...)).
    // Single-value facets (project ids) are sent as-is.
    const opts = {};
    for (const [k, v] of Object.entries(values)) {
      if (Array.isArray(v)) { if (v.length) opts[k] = v; }
